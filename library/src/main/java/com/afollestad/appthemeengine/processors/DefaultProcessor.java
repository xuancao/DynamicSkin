...
public class DefaultProcessor implements Processor<View, Void> {
    ...
    private void processTagPart(@NonNull Context context, @NonNull View current, @NonNull String tag, @Nullable String key) {
        switch (tag) {
            ...
            //TODO
            case KEY_TEXT_SELECT_PRIMARY_COLOR:
                ((TextView) current).setTextColor(getTextSelectSelector(Config.primaryColor(context, key), current));
                break;
    ...
    //TODO--
    private static ColorStateList getTextSelectSelector(@ColorInt int color, View view) {
        return new ColorStateList(new int[][]{
                new int[]{-android.R.attr.state_selected},
                new int[]{android.R.attr.state_selected}
        }, new int[]{
                view instanceof Button ? Color.BLACK : Color.parseColor("#666666"),
                color
        });
    }
    ...
    private final static String KEY_TEXT_SELECT_PRIMARY_COLOR = "text_select_primary_color";
